...
void PCA9685_Init(PCA9685 *board) {
	...
	PCA9685_set_prescaler(board, (board->prescaler) ? board->prescaler : 121);	// Set prescaler (default: 50MHz TODO: Calculate)
...
// Calculates the servo's value based on its `range_min` and `range_max` using the target (between `SERVO_MIN` and `SERVO_MAX`)
...
void Servo_update(Servo *servo) {
	if (servo->value > SERVO_MAX) servo->value = SERVO_MAX;
	if (servo->value < SERVO_MIN) servo->value = SERVO_MIN;
