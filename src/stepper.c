...
bool valid_stepper_io(StepperIO *io) {
  return (io->step && io->dir && io->reset && io->enable && io->ms1 &&
          io->ms2 && io->ms3);
}

bool valid_stepper_simplified_io(StepperIO *io) {
  return (io->step && io->dir);
}
...
void Stepper_set_mode(Stepper *self, StepperMode mode) {
  ...
  HAL_GPIO_WritePin(self->io.gpio, self->io.ms1,
                    (mode & 0b001) ? GPIO_PIN_SET : GPIO_PIN_RESET);
  HAL_GPIO_WritePin(self->io.gpio, self->io.ms2,
                    (mode & 0b010) ? GPIO_PIN_SET : GPIO_PIN_RESET);
  HAL_GPIO_WritePin(self->io.gpio, self->io.ms3,
                    (mode & 0b100) ? GPIO_PIN_SET : GPIO_PIN_RESET);
}

void Stepper_set_direction(Stepper *self, StepperDirection sd) {
  self->dir = sd;
  HAL_GPIO_WritePin(self->io.gpio, self->io.dir,
                    (sd == STEPD_CLOCKWISE) ? GPIO_PIN_RESET : GPIO_PIN_SET);
}
...
void Stepper_init(Stepper *self) {
  ...
  init.Pin = self->io.dir | self->io.enable | self->io.reset | self->io.step |
...
bool Stepper_queue_steps(Stepper *self, uint32_t step_count,
  ...
  for (int i = 0; i < MAX_STEPPERS; i++) {
    ...
    if (stepper_queue[i].stepper == self || stepper_queue[i].num_steps == 0 ||
        stepper_queue[i].stepper == NULL) {
      // Update the entry
      stepper_queue[i] = (StepperQueueEntry){.stepper = self,
...
void StepperQueueEntry_operate(StepperQueueEntry *entry) {
  ...
  if (stepper && entry->num_steps > 0) {
    ...
    if (entry->is_high) {
      ...
      stepper->position +=
          (stepper->dir == STEPD_FORWARDS) ? um_per_step : -um_per_step;
    }
