...
void USART2_IRQHandler() {
  if (USART2->SR & 0x0020) {
    ...
    // Null terminator
    USART2_Buffer[(end_idx + 1) % BUF_MAX] = 0;
  }
}

int USART2_read_IRQ_buffer(char *into, int len) {
  int i = 0;
  while (i < (len - 1) && start_idx != end_idx) {
    into[i] = USART2_Buffer[start_idx];
...
int USART2_read_IRQ_buffer_until(char *into, int len, char terminator) {
  int i = 0;
  while (i < (len - 1) && start_idx != end_idx) {
    while (USART2_Buffer[start_idx] == '\0')
      ;
    char c = USART2_Buffer[start_idx];
    into[i] = c;
    start_idx = (start_idx + 1) % BUF_MAX;
    i++;
    if (c == terminator) {
...
int USART2_buffer_content_len() {
  return (end_idx > start_idx) ? end_idx - start_idx
                               : (end_idx + BUF_MAX) - start_idx;
}
#endif
...
inline bool USART_has_data(USART u) { return (u->SR & 0x0020) != 0; }
...
#ifdef USE_IRQ
  if (cookie == USART2) {
    DBG("Reading from IRQ Buffer");
    ssize_t x = USART2_read_IRQ_buffer_until(buf, size, '\n');
    char s[256];
    sprintf(s, "Got: %s", buf);
    DBG(s);
		return x;
...
FILE *USART_fopen(USART u) {
  // Open a USART device as a file with write permissions, returning a file
  // handle that allows the USART device to be read from and written to. Only
  // works with GCC due to requiring the cookies extension for custom streams.
  // Thankfully, PlatformIO uses GCC.

  // Create the cookie functions
  cookie_io_functions_t c = {.read = USART_Cookie_read,
